...
export default {
  ...
  methods: {
    ...
    addPayment () {
      const {
        id, value, category, date, paymentDay
      } = this
      const data = {
        id,
        value,
      ...
      console.log('add payment: ' + data.category + data.value + data.date)
      if (this.checkInputs() === true) {
        this.$emit('add-payment', data)
        this.value = this.category = this.date = ''
        this.$route.params.category = this.$route.query.value = ''
      }
    },
    getRouteData (data) {
      console.log('get route data: ' + data.category + ' ' + data.value)
      if (data.category) {
        this.show = true
  ...
  mounted () {
    this.getRouteData(this.formData)
  }
}
</script>
